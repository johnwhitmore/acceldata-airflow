...
#     inputs=[Node(job_uid='create_run_id')],
...
def read_data(**context):
    ...
    for obj in prefix_objs:
        if obj.key.endswith('.zip'):
            ...
            out_buffer.seek(0)
            lab_s3.Object(minio_bucket, parquet_key).put(Body=out_buffer.read())
...
def aggregate_rides_data(**context):
    ...
    for obj in prefix_objs:
        if obj.key.endswith('.parquet'):
            ...
            out_buffer.seek(0)
            lab_s3.Object(minio_bucket, daily_key).put(Body=out_buffer.read())
